...
contract UpVsDownGameV2 is Ownable {
    ...
    uint256 time;
    event TimeStamps(uint256 time1, uint256 start);
        ...
        onlyPoolExists(userTrade.poolId)
    {
        ...
        emit TimeStamps(time, pools[userTrade.poolId].tradesEndTimeMS);
        emit TimeStamps(time, pools[userTrade.poolId].tradesStartTimeMS);

        //Prevent making trade while end round transaction being confirmed on blockchain
        require(
            time <= pools[userTrade.poolId].tradesEndTimeMS,
            "Round is closing"
        );

        //Prevent making trade while round starts
        require(
            time >= pools[userTrade.poolId].tradesStartTimeMS,
